...
class SPARouter {
  ...
  /**
   * Компилирует маршруты с параметрами в regex паттерны
   * /user/{id} -> /user/(\w+) с именем параметра 'id'
   */
  compileRoutes() {
    this.routes = this.routesConfig.routes.map((route) => {
      ...
      // Извлекаем имена параметров из {paramName}
      ...
      // /user/{id} -> /user/(?<id>[^/]+)
      // /blog/{slug}/comment/{id} -> /blog/(?<slug>[^/]+)/comment/(?<id>[^/]+)
  ...
  /**
   * Парсит URL и возвращает параметры
   * /user/123 с паттерном /user/{id} -> { id: '123' }
   */
  ...
  /**
   * Ищет маршрут по пути с поддержкой динамических параметров
   */
  getRouteConfig(path) {
    const p = this.normalizePath(path);

    // Сначала ищем точное совпадение (быстрее)
  ...
  renderPage(pageData) {
    ...
    // Передаем параметры в window для доступа со страницы
    ...
    // Вызываем hook для инициализации логики страницы с параметрами
